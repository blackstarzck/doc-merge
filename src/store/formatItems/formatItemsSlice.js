...
const initialState = formatAdapter.getInitialState(
  ...
  localStorageFormatItems ? JSON.parse(localStorageFormatItems) : DEFAULT_FORMAT_ITEMS
)
...
export const formatItemsSlice = createSlice({
  ...
  reducers: {
    ...
    addFormatItem: (state, action) => {
      ...
      localStorage.setItem(LOCAL_STORAGE_FORMAT_ITEM_KEY, JSON.stringify(state.entities))
    },
...
export const { setFormatItems, addFormatItem, removeFormatItem, updateFormatItem } = formatItemsSlice.actions

// Selectors
export const { selectAll: selectAllFormatItems, selectById: selectFormatItemByKey } = formatAdapter.getSelectors((state) => state.formatItems)
